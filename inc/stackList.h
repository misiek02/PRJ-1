...
template <typename T> void StackList<T>::push(const T &v) {
  // next = top
  // value = v
...
template <typename T> T StackList<T>::pop() {
...
template <typename T> bool StackList<T>::empty() {
  if (s_top == NULL) {
...
template <typename T> const T &StackList<T>::top() {
