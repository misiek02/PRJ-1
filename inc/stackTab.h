#pragma once
template <typename T> class StackTab {
...
public:
  ...
  StackTab(std::size_t initialCapacity) {
...
template <typename T> void StackTab<T>::push(const T &value) {
  if (s_top == capacity - 1) {
    std::cout << "Overflow\n";
    exit(1);
  }
...
template <typename T> std::size_t StackTab<T>::size() {
  if (s_top == -1) {
...
template <typename T> const T &StackTab<T>::top() {
